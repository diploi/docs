...
import { GifImage } from '../components/images';

<GifImage src="/GetStartedTodoApp.gif" alt="Next.js Todo app example" />
...
In order to run your deployments you need some computing power, for which we will launch a Core.
Internally a Core is a Kubernetes cluster, but don't worry, you don't need to know anything about
Kubernetes in order to use Diploi. You can think of a Core as storage and computing power to
run your workloads on.

<Image
  src="/GetStartedCore.gif"
  alt="Add core example"
  width={640}
  height={360}
...
Click on the Core icon in the top bar and from there **Setup Core**. If you are on the free plan there is
only one size of Cores to choose and it will be preselected, so you can just click **Launch Diploi Core**.

import { Callout } from 'nextra-theme-docs';

<Callout type="info" emoji="ℹ️">
  The only difference between a paid and a Free Core is that a Free Core will
  only stay on for 8 hours, and after that you will need to restart it from the
  control panel.
</Callout>
...
In the project setup screen, we don't need to change anything, but here you could choose e.g.
which Core you want the project to run on or how to connect to the database.

<Image
  src="/GetStartedProject.gif"
  alt="Create a project example"
  width={640}
  height={360}
  style={{ marginTop: '30px;', borderRadius: '10px' }}
  unoptimized={true}
...
<Image
  src="/GetStartedDeployment.gif"
  alt="Create a deployment example"
  width={640}
  height={360}
  style={{ marginTop: '30px;', borderRadius: '10px' }}
  unoptimized={true}
