...
export const bunDetails = extractElement(AllElements,'bun')
export const bunPackage = repoURLPackageBuilder(bunDetails.url,bunDetails.versions.pop())
...
<Code code={`components:
  - name: ${bunDetails.name}
    identifier: ${bunDetails.identifier}
    package: ${bunPackage}`} lang="yaml" />
