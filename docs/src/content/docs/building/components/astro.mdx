...
export const astroDetails = extractElement(AllElements,'astro')
export const astroPackage = repoURLPackageBuilder(astroDetails.url,astroDetails.versions.pop())
...
<Code code={`components:
  - name: ${astroDetails.name}
    identifier: ${astroDetails.identifier}
    package: ${astroPackage}`} lang="yaml" />
