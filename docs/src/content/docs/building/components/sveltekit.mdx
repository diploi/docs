...
export const sveltekitDetails = extractElement(AllElements,'sveltekit')
export const sveltekitPackage = repoURLPackageBuilder(sveltekitDetails.url,sveltekitDetails.versions.pop())
...
<Code code={`components:
  - name: ${sveltekitDetails.name}
    identifier: ${sveltekitDetails.identifier}
    package: ${sveltekitPackage}`} lang="yaml" />
