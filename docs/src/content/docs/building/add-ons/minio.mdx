...
export const minioDetails = extractElement(AllElements,'minio')
export const minioPackage = repoURLPackageBuilder(minioDetails.url,minioDetails.versions.pop())
...
<Code code={`addons:
  - name: ${minioDetails.name}
    identifier: ${minioDetails.identifier}
    package: ${minioPackage}`} lang="yaml" />
