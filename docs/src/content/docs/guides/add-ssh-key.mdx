...
<Tabs syncKey="os">
  <TabItem label="Linux">
   <Steps>
   1. Open a terminal and run: `ssh-keygen` which generate by default a ed25519 key pair
   2. You will get the message, asking you to assign a name for the key pair
      ```bash
  ...
  <TabItem label="Windows">
   <Steps>
   1. Using Git Bash, Terminal or WSL run: `ssh-keygen` which generate by default a ed25519 key pair
   2. You will get the message, asking you to assign a name for the key pair
      ```bash
      Generating public/private ed25519 key pair.
      Enter file in which to save the key (/home/<your_username>/.ssh/id_ed25519): 
  ...
  <TabItem label="Mac">
   <Steps>
   1. Open Terminal and run: `ssh-keygen` which generate by default a ed25519 key pair
   2. You will get the message, asking you to assign a name for the key pair
      ```bash
      Generating public/private ed25519 key pair.
      Enter file in which to save the key (/home/<your_username>/.ssh/id_ed25519): 
  ...
  <TabItem label="Linux">
   <Steps>
      1. ssh-keygen generates two files, one with an extension `.pub` and the other without any extension, eg. `someKey.pub` and `someKey`. The one we need right now is `someKey.pub`, which is your public key. You need to copy the contents of your `.pub` file. You can use the `cat` command:
         ...
         cat someKey.pub
         ```
      2. By using `cat` you will be print the content of the `someKey.pub` file, it will look like this:
         ```bash
...
Now that you copied the contents of your public key, you need to add it your Diploi profile:
<Steps>
   ...
   5. Click the "Add SSH Key" button.
      <Image src={AddSSHKey} alt="Add SSH Key"/>
</Steps>
